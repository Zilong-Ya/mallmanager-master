...
<el-card shadow="always">
  <!-- 1.面包屑 -->
  <com-breadcrumb levelName1="商品管理" levelName2="修改商品"></com-breadcrumb>

  <!-- 2.提示组件 -->
  <el-alert title="修改商品信息" type="success" center show-icon></el-alert>
...
export default {
  ...
  created() {
    ...
    this.$http.get(`goods/${goodsId}`)
        .then(res=>{
          ...
          this.formdata.goods_name=goods.goods_name,
          this.formdata.goods_price=goods.goods_price,
          this.formdata.goods_number=goods.goods_number,
          this.formdata.goods_weight=goods.goods_weight,
          this.formdata.goods_introduce=goods.goods_introduce,
          // 转换分类信息 
          this.selectOptionsValues = goods.goods_cat.split(',').map(item => parseInt(item))
          // 显示图片列表
          goods.pics.map(item=>{
            // 显示列表
            this.fileList.push({name:item.pics_id,url:item.pics_big_url})
          })
        })
  ...
  methods: {
    ...
    async handleEditGoods(){ // 添加商品
      ...
      let res = await this.$http.put(`goods/${this.formdata.goods_id}`,this.formdata)
